...
class ST7789(framebuf.FrameBuffer):
    def __init__(
        self, spi, width=240, height=135, reset=None, dc=None, cs=None, backlight=None
    ):
        self.width = 240
        self.height = 135
        self.spi = spi
    ...
    def set_rotation(self, rotation):
        """0: Landscape, 1: Landscape Flipped 180"""
        if rotation == 0:
            self._write(0x36, b"\x78")  # BGR
            self.x_offset = 40
            self.y_offset = 53
        else:
            self._write(0x36, b"\xb8")  # MY=1, MX=1 (180° flip)
            self.x_offset = 40
            self.y_offset = 52
        self.show()
        ...
        self._write(
            ...
            bytearray(
                [
                    0,
                    self.x_offset,
                    (self.width + self.x_offset - 1) >> 8,
                    (self.width + self.x_offset - 1) & 0xFF,
                ]
            ),
        )
        self._write(
            ...
            bytearray(
                [
                    0,
                    self.y_offset,
                    (self.height + self.y_offset - 1) >> 8,
                    (self.height + self.y_offset - 1) & 0xFF,
                ]
            ),
        )
